...
## Create and validate a mode-0600 Redis RDB snapshot under ./backups, then keep only
...
## 01:10 host time; SCHEDULE='0 2 * * *' picks another. Every app on the
...
restore:
	...
	@test -n "$(BACKUP)" || (echo "Refusing restore: pass BACKUP=/absolute/path/file.rdb" >&2; exit 1)
