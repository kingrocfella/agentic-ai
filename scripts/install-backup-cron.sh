...
#
#   ./scripts/install-backup-cron.sh                     # daily at 01:10
...
# UTC server that is 01:10 UTC.
set -eu

repo_dir=$(CDPATH= cd -- "$(dirname -- "$0")/.." && pwd)
schedule=${SCHEDULE:-'10 1 * * *'}
marker='# agentic-ai: daily database backup (scripts/install-backup-cron.sh)'
